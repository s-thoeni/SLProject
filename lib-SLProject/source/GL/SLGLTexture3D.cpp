//
// Created by thoeni on 9/29/2018.
//

#include <SLGLTexture3D.h>
#include <SLGLState.h>

SLGLTexture3D::SLGLTexture3D(const std::vector <GLfloat> &textureBuffer,
                             SLint width,
                             SLint height,
                             SLint depth,
    ...
    const int levels = 7;
    glTexStorage3D(GL_TEXTURE_3D, levels, GL_RGBA8, _width, _height, _depth);
    GET_GL_ERROR;
    ...
    glTexSubImage3D(GL_TEXTURE_3D, 0, 0, 0, 0, _width, _height, _depth, GL_RGBA, GL_FLOAT, &textureBuffer[0]);
    GET_GL_ERROR;
...
void SLGLTexture3D::activate(SLint shaderProgram, SLstring glSamplerName, SLint textureUnit) {
